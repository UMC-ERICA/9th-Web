...
function useThrottle<T>(value:T, delay=500):T {
    //1. 상태 변수: throttledValue : 최종적으로 쓰로롤링 적용된 값
    // 초기값을 전달받은 value
    ...
    const lastExcuted: React.RefObject<number> = useRef<number>(0);
    ...
    useEffect(() => {
        ...
        // CleanUp Function 이펙트가 재실행되기 전에 타이머ㄹ가 실행도지 않았다면
        // 기존 타이머를 clearTimeout을 통해 취소하여 중복 어ㅂ데이트를 방지
